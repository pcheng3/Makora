...
- **Human-in-the-loop learning** — Rate each finding (thumbs up/down + optional comment). After enough ratings, Mahoraga synthesizes rules and few-shot examples that shape future reviews
...
4. **Mahoraga learns** — Hit "Learn from Ratings" and the learning pipeline extracts patterns into rules. Similar rules are automatically clustered (Jaccard similarity) and merged by the AI.
...
2. Click **Export** to download `mahoraga-rules.json`
